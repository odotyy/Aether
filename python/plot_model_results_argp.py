...
#-----------------------------------------------------------------------------
# A few things to note about using argparser:
# Can't do the file list at the end the way originally done,
#
#
#---------------------------------------------------------------------------
...
def get_args(argv):
    ...
    parser = argparse.ArgumentParser(description = """Parse the arguments and set to a dictionary containing 
            ...
            lon (to plot), IsLog, and help (display help) """, usage= """gitm_plot_one_alt.py -var=N 
                ...
                -help [*.bin or a file]""" , epilog= """ At end, enter -filelist and list the file you want to plot.
    ...
    parser.add_argument('-cut', metavar = 'cut',  default ='alt', nargs= 1, \
                        help = '   -cut=alt,lat,lon : which cut you would like')
    parser.add_argument('-winds', default =False,\
                        help='-winds: overplot winds', \
                        action="store_true")
    parser.add_argument('-alt', metavar = 'alt', default =400.0, nargs= 1,\
                        help = '   -alt=altitude : can be either alt in km or grid number (closest)')
    parser.add_argument('-lat', metavar = 'lat',  default =-100.0, nargs= 1, \
                        help = '   -lat=latitude : latitude in degrees (closest)')
    parser.add_argument('-lon', metavar = 'lon',  default =-100.0, nargs= 1,\
    ...
    parser.add_argument('-diff', metavar = 'diff',  default =0, \
                        help = '-diff: difference with other plots (int)')
    ...
    parser.add_argument('-movie', metavar = 'movie',  default =0,\
                        action='store_const', const = '1', help = ' -movie: ')
    parser.add_argument('-tec', metavar = 'tec',  default =0, \
                        action='store_const', const = '1', help = ' -tec: ')
    parser.add_argument('-rate', metavar = 'rate',  default =30, nargs= 1,\
                        help = ' -rate: framerate for movie')
    ...
    parser.add_argument('filelist', nargs='+', \
                        help = '   filelist: list files to use for generating plots')
...
if len(args.filelist) > 0 and not(len(sys.argv) == 0):
                m = re.match(r'(.*)bin',args.filelist[0])
                if m:
                    IsGitm = 1
                    HasHeader = 0
        ...
        if (cut == 'alt'):
            ...
            if (len(Alts) > 1):
                if (int(args.alt) < 50):
                    ...
                    if (int(args.alt) > Alts[nAlts-3]):
                        iAlt = nAlts-3
                    else:
                        iAlt = 2
                        while (Alts[iAlt] < int(args.alt)):
                            iAlt=iAlt+1
            else:
                iAlt = 0
            Alt = Alts[iAlt]
